...
function useWorkspaceFileImport() {
  ...
  const importFiles = useCallback(
    async (acceptedFiles) => {
      ...
      if (wm) {
        const count2 = state.sessionImages.length;
        const room2 = remainingWorkspaceSlots(count2);
        if (room2 <= 0) {
      ...
      try {
        ...
        const eligible = sniffRows.filter((r2) => r2.ok).map((r2) => r2.file);
        const typeRejected = sniffRows.filter((r2) => !r2.ok).map((r2) => r2.file.name);
        if (typeRejected.length > 0) {
          toast.warning(`Skipped ${typeRejected.length} non-image file(s)`, {
            description: typeRejected.slice(0, 5).join(" \xB7 ") + (typeRejected.length > 5 ? " \u2026" : "")
          });
        }
        ...
        if (isStorageOnlyMode()) {
          ...
          const results2 = rows.filter((r3) => r3.ok).map((r3) => r3.img);
          const failures = rows.filter((r3) => !r3.ok).map((r3) => r3.name);
          if (failures.length > 0) {
            toast.warning(`Couldn\u2019t import ${failures.length} file(s)`, {
              description: failures.slice(0, 6).join(" \xB7 ") + (failures.length > 6 ? " \u2026" : "")
            });
          }
          if (results2.length === 0) return null;
          if (!useImageStore.getState().workspaceMode) {
            useImageStore.getState().setStandardImport(results2[0]);
            notifyImportToast(null, overflow, results2.length, "standard");
            return { added: 1, droppedDueToCapacity: 0, duplicatesSkipped: 0 };
          }
          const r2 = useImageStore.getState().addImagesToSession(results2);
          notifyImportToast(r2, overflow, results2.length, "workspace");
          return r2;
        }
...
function notifyImportToast(r, overflowFromSlice, uploadedLen, mode) {
  ...
  const description = descParts.length > 0 ? descParts.join(" \xB7 ") : void 0;
  if (r.added > 0) {
