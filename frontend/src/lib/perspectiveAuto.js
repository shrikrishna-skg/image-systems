...
function estimateAutoRotationRad(bmp, mode) {
  const iw = bmp.width;
  const ih = bmp.height;
...
function estimateAutoRotationRadCenterAngle(bmp) {
  const v = estimateAutoRotationRad(bmp, "verticals");
  const h = estimateAutoRotationRad(bmp, "horizon");
  if (v === 0 && h === 0) return 0;
  if (v === 0) return h;
  if (h === 0) return v;
  return Math.abs(v) >= Math.abs(h) ? v : h;
}
export {
  estimateAutoRotationRad,
  estimateAutoRotationRadCenterAngle
};
