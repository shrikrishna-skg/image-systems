...
function warnIfApiUnreachable(apiOrigin) {
    ...
    server: {
      ...
      proxy: {
        "/api": {
          ...
          configure(proxy) {
            proxy.on("error", (err) => logProxyError("/api", err));
          },
        },
        "/uploads": {
          target: apiProxyTarget,
          ...
          proxyTimeout: longTimeout,
          configure(proxy) {
            proxy.on("error", (err) => logProxyError("/uploads", err));
          },
        },
      },
    },
